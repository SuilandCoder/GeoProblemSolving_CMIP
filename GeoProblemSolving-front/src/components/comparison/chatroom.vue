      ...
      <transition name="right">
        <div class="searchPanel" v-show="searchPanelShow">
          ...
          <div class="message_record_page">
            <Page :current="currentPage" :total="totalMsg" :page-size="pageSize" @on-change="changeRecordPage" simple
              ref="page" />
...
export default {
  ...
  methods: {
    ...
    getGroupId() {
      ...
      // }
      this.scopeType = "project";
      this.scopeId = this.$route.query.groupID;
    },
    ...
    async getGroupInfo() {
      ...
      if (scopeId != undefined && scopeId != "") {
        let data = await this.axios.get(
          `/GeoProblemSolving_Backend/cmp_project/getProjectAllInfo?projectId=${scopeId}`
        );
        ...
        if (data && data.status === 200) {
          ...
          if (res.code === 0 && res.data) {
            ...
            this.$set(this, "groupInfo", project);
            console.log("groupInfo:", this.groupInfo);
            this.selectGroup.selectName = project.title;
            this.selectGroup.selectId = project.projectId;
    ...
    async getParticipants() {
      ...
      for (let i = 0; i < membersList.length; i++) {
        ...
        let data = await this.axios.get(
          `/GeoProblemSolving_Backend/user/inquiry?key=userId&value=${userId}`
        );
    ...
    showRecords() {
      ...
      if (this.searchPanelShow === true) {
        this.axios
          .get(
            "/GeoProblemSolving_Backend/message/inquiry" +
            "?type=message" +
            "&key=roomId" +
            "&value=" +
